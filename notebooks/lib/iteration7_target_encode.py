"""Tier 2a - replace the one-hot Section_ID with a smoothed per-section class profile.
...
VARIANTS = {
    ...
    "+ section profile":           dict(base="full", enc=[("section", section)]),
    "section profile REPLACES 1h": dict(base="drop", enc=[("section", section)]),
    "+ section + mouse + segment": dict(base="full", enc=[("section", section),
                                                          ("mouse", mouse),
                                                          ("segment", segment)]),
    "+ random group (null)":       dict(base="full", enc=[("random", random_group)]),
}
...
def run(tag, spec):
    ...
    for f, (tr, va) in enumerate(folds):
        blocks_tr = [core[tr], OTHER[tr]]
        blocks_va = [core[va], OTHER[va]]
        for _, groups in spec["enc"]:
            e_tr, e_va = target_encode(groups, tr, va)
            blocks_tr.append(e_tr)
            blocks_va.append(e_va)
...
results = {name: run(name, spec) for name, spec in VARIANTS.items()}
...
for i, r in enumerate(rows):
    ...
    print(f"  {r['variant']:30s} {r['gain_vs_baseline']:+.4f} p={r['p_vs_baseline']:.3g} "
...
print(f"\n  VERDICT: {'ADOPT' if any(r['passes'] for r in rows) else 'DO NOT ADOPT'} "
      f"(pre-registered: Holm p<0.05 vs baseline AND beat the random-grouping null)",
      flush=True)
pd.DataFrame(rows).to_csv(OUT / "target_encode.csv", index=False)
print(f"\nwrote {OUT/'target_encode.csv'}", flush=True)
